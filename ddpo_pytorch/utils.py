...
@torch.no_grad()
def sd_sample(
    prompts, pipe, height, width, guidance_scale, num_inference_steps, eta, device
):
    ...
    )  # (8, 77, 768) The 8 is because of duplication
    ...
    for i, t in enumerate(progress_bar(scheduler.timesteps)):
        ...
        pred = unet(
            input, t, encoder_hidden_states=text_embeddings
        ).sample  # (8, 4, 64, 64) noise
    ...
    def update(self, prompts, rewards):
        ...
        for prompt in unique:
            ...
            advantages[prompts == prompt] = (
                prompt_rewards - mean
            ) / std  # advantage is normalized rewards
...
def sample_and_calculate_rewards(
    ...
    for a batch
    """
    preds, all_step_preds, log_probs = sd_sample(
        prompts, pipe, image_size, image_size, cfg, num_timesteps, 1, device
    )
